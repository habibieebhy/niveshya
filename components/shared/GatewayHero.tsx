...
export default function GatewayHero() {
  // State to handle logo image loading failures cleanly without direct DOM manipulation
  ...
  return (
    <main className="relative min-h-screen pb-24">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12">
        ...
        {/* 2. Place the slider at the very top of your content */}
        <GrandHeroSlider />
        ...
        <div className="whitespace-nowrap flex overflow-hidden">
          <div className="animate-marquee items-center">
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((num) => (
              <div key={`set1-${num}`} className="mx-12 text-3xl font-bold text-muted-foreground/40 hover:text-muted-foreground transition-colors cursor-default">
                COMPANY {num}
              </div>
            ))}
            {/* Duplicate set for seamless infinite looping */}
            {[1, 2, 3, 4, 5, 6, 7].map((num) => (
              <div key={`set2-${num}`} className="mx-12 text-3xl font-bold text-muted-foreground/40 hover:text-muted-foreground transition-colors cursor-default">
                COMPANY {num}
              </div>
            ))}
          </div>
